...
hx = int(float(sys.argv[3]) * 100) if len(sys.argv) > 3 else 100
...
pattern = f"EigenSpectrum_{N}_{theta}_{hx}_{hy}_*.h5"
...
combined_output = os.path.join(
    combined_dir,
    f"combined_energy_data_{theta}_{hx}_{hy}_{N}.txt"
)

derivative_output = os.path.join(
    derivative_dir,
    f"second_derivative_energy_data_{theta}_{hx}_{hy}_{N}.txt"
)
...
with open(combined_output, "w") as outfile:
    ...
    for filepath in file_paths:
        try:
            ...
            outfile.write(
                f"{hx/100.0} {hy/100.0} {hz} {theta/100.0} {ground_energy}\n"
            )
...
with open(derivative_output, "w") as outfile:
    for hz, d2e in zip(hz_values, second_derivative):
        outfile.write(
            f"{hx/100.0} {hy/100.0} {hz} {theta/100.0} {d2e}\n"
        )
