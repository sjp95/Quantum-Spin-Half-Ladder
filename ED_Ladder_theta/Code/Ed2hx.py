...
theta = int(float(sys.argv[2]) * 100) if len(sys.argv) > 2 else 100
hy = int(float(sys.argv[3]) * 100) if len(sys.argv) > 3 else 100
hz = int(float(sys.argv[4]) * 100) if len(sys.argv) > 4 else 100
...
pattern = f"EigenSpectrum_{N}_{theta}_*_{hy}_{hz}.h5"
...
combined_output = os.path.join(
    combined_dir,
    f"combined_energy_data_{theta}_{hy}_{hz}_{N}.txt"
)

derivative_output = os.path.join(
    derivative_dir,
    f"second_derivative_energy_data_{theta}_{hy}_{hz}_{N}.txt"
)
...
with open(combined_output, "w") as outfile:
    ...
    for filepath in file_paths:
        try:
            ...
            outfile.write(
                f"{hx} {hz/100.0}  {hy/100.0} {theta/100.0} {ground_energy}\n"
            )
...
second_derivative = second_derivative_4th_order(hx_values, energy_values)
...
with open(derivative_output, "w") as outfile:
    for hx, d2e in zip(hx_values, second_derivative):
        outfile.write(
            f"{hx} {hz/100.0} {hy/100.0} {theta/100.0} {d2e}\n"
        )
