...
pattern = f"EigenSpectrum_{N}_{J1}_{J2}_*_{hy}_{hz}.h5"
...
# ============================================================
# Extract hx from filename
# ============================================================

def extract_hx(filepath):
    name = os.path.basename(filepath)
    parts = name.replace(".h5", "").split("_")
    return int(parts[-3])

file_paths.sort(key=extract_hx)
...
combined_output = os.path.join(
    combined_dir,
    f"combined_energy_data_{J1}_{J2}_{hy}_{hz}_{N}.txt"
)

derivative_output = os.path.join(
    derivative_dir,
    f"second_derivative_energy_data_{J1}_{J2}_{hy}_{hz}_{N}.txt"
)
...
with open(combined_output, "w") as outfile:
    ...
    for filepath in file_paths:
        try:
            ...
            outfile.write(
                f"{hx} {hz/100.0}  {hy/100.0} {J1/100.0} {J2/100.0} {ground_energy}\n"
            )
...
with open(derivative_output, "w") as outfile:
    for hx, d2e in zip(hx_values, second_derivative):
        outfile.write(
            f"{hx} {hz/100.0} {hy/100.0} {J1/100.0} {J2/100.0} {d2e}\n"
        )
