...
def main():
    ...
    # Match system output style
    print("===============================")
    print("4-Leg Spin Ladder System")
    print(f"N: {N}")
    print(f"hx: {hx}")
    print(f"hy: {hy}")
    print(f"hz: {hz}")
    ...
    # 1. Build coupling matrices Jx and Jz for 4-leg ladder
    # Number of legs Ny = 4
    Ny = 4
    Nx = N // Ny
    ...
    periodic_leg = (bc.lower() == "pbc")

    for k in range(N):
        x = k // Ny
        y = k % Ny

        # Neighbor along leg (x-direction)
        xp = (x + 1) % Nx
        kxp = xp * Ny + y

        if periodic_leg or x < Nx - 1:
            Jx_mat[k, kxp] = J1
            Jx_mat[kxp, k] = J1

        # Neighbors along rung (y-direction)
        # Open rung boundary conditions: y to y+1 for y in 0, 1, 2
        if y < Ny - 1:
            kyp = x * Ny + (y + 1)
            Jz_mat[k, kyp] = J2
            Jz_mat[kyp, k] = J2
