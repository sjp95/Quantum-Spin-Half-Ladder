...
hz = energy_data[:, 0]
ground_energy = energy_data[:, 3]
second_derivative = derivative_data[:, 3]
...
plt.title(rf"N={N}, J1={J1/100.0}, J2={J2/100.0}")
plt.grid(True)
plt.tight_layout()
...
plt.savefig(
    os.path.join(plot_dir, f"GroundStateEnergy_{N}_{J1}_{J2}.png"),
    dpi=300
)
...
plt.title(rf"Second Derivative, N={N}")
plt.grid(True)
plt.tight_layout()

plt.savefig(
    os.path.join(plot_dir, f"SecondDerivative_{N}_{J1}_{J2}.png"),
    dpi=300
)
